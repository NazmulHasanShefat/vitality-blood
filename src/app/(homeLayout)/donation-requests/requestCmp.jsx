...
import { HiOutlineLocationMarker, HiOutlineCalendar, HiOutlineHeart } from "react-icons/hi";
import { MdOutlineMessage, MdVerifiedUser } from "react-icons/md";

// Dummy datasets based on your requirement to map
const bloodGroups = ["All Groups", "A+", "A-", "B+", "B-", "O+", "O-", "AB+", "AB-"];
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <div className="relative">
                <select className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-red-600 appearance-none cursor-pointer">
                  {bloodGroups.map((group, index) => (
                    <option key={index} value={group}>{group}</option>
                  ))}
                </select>
                <span className="absolute right-3 top-3.5 pointer-events-none text-xs text-slate-400">▼</span>
              </div>
            </div>
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <div className="relative">
                <select className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg pl-9 pr-8 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-red-600 appearance-none cursor-pointer">
            ...
            {/* Upazila Dropdown */}
            <div className="flex flex-col gap-1.5">
              <label className="text-xs font-semibold text-slate-500 dark:text-slate-400">Upazila</label>
              <div className="relative">
                <select className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg pl-9 pr-8 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-red-600 appearance-none cursor-pointer">
            ...
            {/* Filter Apply Button */}
            <button className="w-full flex items-center justify-center gap-2 bg-slate-900 hover:bg-slate-800 dark:bg-slate-800 dark:hover:bg-slate-700 text-white font-medium py-2.5 rounded-lg transition-colors shadow-sm">
              <BiFilterAlt />
      ...
      <div className="max-w-7xl mx-auto px-4 py-6">
        ...
        <div className="flex justify-between items-center mb-6">
          <div className="text-lg font-semibold text-slate-700 dark:text-slate-300">
            Available Donors <span className="text-slate-400 text-sm font-normal">(124 verified)</span>
          </div>
